...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.findOne({ user: req.params.user_id })
    ...
    .catch(err =>
      res.status(404).json({ noprofile: "There's no profile for this user." })
    );
});
...
router.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id }).then(profile => {
      if (!profile)
        return res.json({ noprofile: "There are no profiles with this id" });
      // Get remove index
